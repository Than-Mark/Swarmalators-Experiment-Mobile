...
class PathPlanningSw(Swarmalators2D):
    def __init__(self, K: float, Jsw: float, Jpin: float,
                 pinPosition: np.ndarray,
        ...
        self.J = np.concatenate([np.full(agentsNum, Jsw), np.full(pinPosition.shape[0], Jpin)], axis=0)
        self.Jsw = Jsw
        self.Jpin = Jpin
        self.one = np.ones((agentsNum, agentsNum))
        self.pinPosition = pinPosition
        self.positionX = np.concatenate([self.positionX, pinPosition], axis=0)
        self.speedMask = np.concatenate([np.ones(agentsNum), np.zeros(pinPosition.shape[0])], axis=0)
        self.phaseTheta = np.random.random(agentsNum + pinPosition.shape[0]) * 2 * np.pi
        self.update_temp()
    ...
    def calc_dot_position(deltaX: np.ndarray, deltaY: np.ndarray, 
                          ...
                          distance: np.ndarray, speedMask: np.ndarray) -> tuple:
        ...
        return dotX * speedMask, dotY * speedMask

    @property
    def dotTheta(self) -> np.ndarray:
        return self._calc_dot_theta(self.temp["deltaTheta"], self.omega, self.K)

    @staticmethod
    @nb.njit
    def _calc_dot_theta(deltaTheta: np.ndarray, omega: np.ndarray, K: float) -> np.ndarray:
        coupling = np.zeros(deltaTheta.shape[0])
        for idx in range(deltaTheta.shape[0]):
            coupling[idx] = np.mean(np.sin(deltaTheta[idx]))
    ...
    def plot(self, ax: plt.Axes = None) -> None:
        ...
        ax.set_title(r"$J_{\text{sw}}=" + rf"{self.Jsw:.2f}" +
                     r",\ J_{\text{pin}}=" +"{self.Jpin:.2f},\ K={self.K:.2f}$")

    def __str__(self) -> str:
        return f"PathPlanningSw_a{self.agentsNum}_K{self.K:.2f}_J{self.J:.2f}"
    

class StateAnalysis:
    def __init__(self, model: PathPlanningSw = None, lookIndex: int = -1, showTqdm: bool = False):
    ...
    @staticmethod
    def calc_order_parameter_R(model: PathPlanningSw) -> float:
        return np.abs(np.sum(np.exp(1j * model.phaseTheta))) / model.agentsNum
    
    @staticmethod
    def calc_order_parameter_S(model: PathPlanningSw) -> float:
    ...
    @staticmethod
    def calc_order_parameter_Vp(model: PathPlanningSw) -> float:
        pointX = model.temp["pointX"]
        phi = np.arctan2(pointX[:, 1], pointX[:, 0])
        return np.abs(np.sum(np.exp(1j * phi))) / model.agentsNum
    
    @staticmethod
    def calc_order_parameter_Ptr(model: PathPlanningSw) -> float:
    ...
    def plot_last_state(self, model: PathPlanningSw = None, ax: plt.Axes = None, withColorBar: bool =True, 
                        s: float = 50) -> None:
        if ax is None:
        ...
        if model is not None:
            ...
            ax.set_title(rf"$J={model.J:.2f},\ K={model.K:.2f}$")
        else:
            ...
            ax.set_title(rf"$J={self.model.J:.2f},\ K={self.model.K:.2f}$")
    ...
    def plot_state_at_step(self, model: PathPlanningSw = None, ax: plt.Axes = None, withColorBar: bool =True, 
                           step: int = 9000, s: float = 50) -> None:
        """
        绘制指定时间步 (step) 的模型状态，如果没有传递 step，则绘制最后一步的状态
        :param model: PathPlanningSw 模型
...
def draw_mp4(model: PathPlanningSw, savePath: str = "./data", mp4Path: str = "./mp4", step: int = 1, earlyStop: int = None):
