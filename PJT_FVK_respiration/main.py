...
class FVK(Swarmalators2D):
    def __init__(self, strengthK: float, distanceD0: float, phaseLagA0: float,
        ...
        if freqDist == "uniform":
            self.freqOmega = np.random.uniform(omegaMin - deltaOmega, omegaMin + deltaOmega, agentsNum)
        elif freqDist == "cauchy":
            self.freqOmega = np.random.standard_cauchy(agentsNum) * deltaOmega + omegaMin
        else:  # freqDist == "gaussian"
            self.freqOmega = np.random.normal(omegaMin, deltaOmega, agentsNum)
...
class StateAnalysis:
    def __init__(self, model: FVK = None):
        if model is None:
            return
        self.model = model
